...
\cabstract{
	...
	1）到目前为止，在弱监督条件下，还没有相关方法直接完成生物标记物精确定位任务，本极具创造性地组合卷积神经网络和对抗生成网络，其中对抗生成网络由生成器和判别器组成，卷积神经网络充当分类器角色。
	
	2）不仅将本文提出的网络结构用于处理二类问题，还将其推广到处理多类问题。
	一方面填补了在弱监督条件下直接完成生物标记物精确定位任务空白，另外一方面，相较于其他间接方法，本文提出的方法则可以通过生成器去除生物标记物，所以生成器输出减去输入即可得到生物标记物的精确位置。本文提出的方法还无须对生物标记物的精确标注，只需要类别标注，就可以得到对应疾病的生物标记物，免去了专业医师标注的标注成本。
