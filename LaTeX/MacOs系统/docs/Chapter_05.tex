\chapter{多类疾病标记物定位}\label{sec:multi_classes}
%\section{前言}
...
从表\ref{tab:simulated_skin_diff_parameters}的平均AUC分数可以看出，本文的默认$\lambda_{1}$和$\lambda_{2}$超参数组合对疾病标记物的定位性能最佳。从各类异常上个AUC分数来看，除了第二类异常外，本文的默认参数均取得了最高AUC分数。我们还能发现，当超参数$\lambda_{1}$偏小时，比如$\lambda_{1}=1$（表中第$3$列），本文提出的模型（以下简称“模型”）取得的平均AUC较低（$0.590$），与默认超参数相比有所下降，极端情况下（$\lambda_{1}=0$）更是如此（表中第$7$列,平均AUC=$0.570$）。这是因为CNN分类器在模型中主要负责定位疾病标记物（实验验证参见\ref{sec:g_c_g_d_g_d_c_comparsion}小节），一旦CNN分类器在模型中占的权重（$\lambda_{1}$）较小，这显然会导致模型定位疾病标记物的定位精确度有所下降。当超参数$\lambda_{2}$偏小时，比如$\lambda_{2}=10$和$\lambda_2=0$，此时L1损失函数所占权重较小，则模型对图像修改的约束将会降低，那么模型在修改疾病标记物所在的像素强度的同时显然会不可避免地修改正常区域的像素强度，因此模型取得的平均AUC更低，平均AUC分别为$0.312$（表中第$5$列）和$0.442$（表中第8列）。反之，当$\lambda_{1}(\ge 4)$和$\lambda_{2}(\le 20)$取值较大时，模型对疾病标记物的定位性能较为稳定，如表中第$2$列（$\lambda_{1}=4, \lambda_{2}=20$）、第$4$列（$\lambda_{1}=10, \lambda_{2}=20$）和$6$列（$\lambda_{1}=4, \lambda_{2}=50$）中模型取得平均AUC分别为$0.668$、$0.659$、$0.628$，可证明模型的超参数适用范围较广，具有较好的鲁棒性。根据三类异常图像所绘制的P-R曲线同样可证明以上结论，图\ref{fig:multi_simulate_pr_curve_image_net_hyper_paras}和图\ref{fig:multi_simulate_pr_curve_circle_hyper_paras}中，绿色P-R曲线明显在其他曲线上方，这说明模型在默认参数下，定位第一类异常和第三类异常的疾病标记物的性能最佳。而在图\ref{fig:multi_simulate_pr_curve_skin_hyper_paras}中，黄色P-R曲线（$\lambda_{1}=10,\lambda_{2}=20$）刚好能够包围其他P-R曲线，其AUC（AUC=$0.823$）也略微大于默认参数得到的AUC分数（AUC=$0.805$）。当超参数$\lambda_{1}$偏小甚至取$0$，图\ref{fig:multi_simulate_pr_curve_image_net_hyper_paras}、图\ref{fig:multi_simulate_pr_curve_skin_hyper_paras}和图\ref{fig:multi_simulate_pr_curve_circle_hyper_paras}中，与绿色P-R曲线（默认参数），深红色P-R曲线（$\lambda_{1}=1$）和粉红色P-R曲线（$\lambda_{1}=0$）的AUC分数均较低，尤其是极端情况$\lambda_{1}=0$（图\ref{fig:multi_simulate_pr_curve_image_net_hyper_paras}中深红色P-R曲线），其AUC最低（AUC=$0.135$），当$\lambda_{2}$取值过小（青色和黑色P-R曲线）或者过大（蓝色P-R曲线）时，情况也类似，与默认参数相比，其AUC也较低，尤其是图\ref{fig:multi_simulate_pr_curve_image_net_hyper_paras}中青色P-R曲线，其AUC最低（AUC=$0.084$）。
%（参见图\ref{fig:multi_simulate_pr_curve_image_net_hyper_paras}、图\ref{fig:multi_simulate_pr_curve_skin_hyper_paras}和图\ref{fig:multi_simulate_pr_curve_circle_hyper_paras}）
\begin{table}[H]
	\centering
	\caption{不同超参数组合下三类异常图像P-R曲线的AUC分数}
