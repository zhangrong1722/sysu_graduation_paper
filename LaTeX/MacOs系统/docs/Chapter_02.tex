...
随着计算机技术在医学影像领域的广泛应用，目前已有可用于疾病标记物定位的方法提出，主要有多示例学习（Multiple Instance Learning，缩写为MIL）、CNN可视化和弱监督目标定位。本小节将选取这三种思路中的部分经典方法进行详细介绍，重点突出这些方法处理疾病标记物定位任务的优势和劣势，力求读者对这些方法有全面而又清晰的认识。
\subsection{多示例学习}
...
\end{equation}
注意，正则项有多种选择，L-2范数通常会将图像平滑。高斯模糊也是常见的正则项，通常会抑制图像中的高频信息。此类方法通常都会固定网络参数，只是根据反向传播算法不断更新输入图像$\ve{x}$，从而合成具有特定模式的输入图像，还能得到类别$c$的原型图。
...
\end{equation}
在梯度回传阶段，梯度可以回传到任意层，任何结构的CNN都会有梯度，故Grad-CAM可可视化任何结构的CNN中的任意层，可以认为Grad-CAM是CAM的泛化。如果要定位图像中的疾病标记物，可将类激活图上采样到原图尺寸大小。注意梯度可以回传到网络浅层（特征图尺寸相对较大），此时上采样到原图时倍数相对较小，所以Grad-CAM相较于CAM可通过可视化浅层特征来得到更加准确的定位结果。
%\begin{figure}[h!] % image examples & compare
...
\begin{figure}[h!]
	...
	\begin{subfigure}{0.57\textwidth}
		...
		\caption{改进版本的对抗性擦除网络~\cite{ZhangWF0H18}}
