...
接下来，本文将给出每个步骤训练时的损失函数。根据等式\ref{equ:model_loss_func}和\ref{item:training_steps}小节中描述的训练策略，本文可进一步分别给出本文两步训练的损失函数。设$\ve{x}$是随机从整个样本空间（包括正常图像和异常图像）中取得的一张图像，对应类别标签为$\ve{y}$。设第一步的损失函数为$L(\ve{x};\vv{\theta})$，则第一步训练的目标是最小化损失函数$L(\ve{x};\vv{\theta})$：
\begin{equation}
L(\ve{x};\vv{\theta}) = \lambda_{1} L_{CE}(G(\ve{x};\vv{\theta})-\ve{x},\ve{y}) + \lambda_{2}L_{ED}(G(\ve{x};\vv{\theta}), \ve{x}).
\end{equation}
...
\begin{algorithm}[h]
	...
	\nl \For{$k\leftarrow 1$ \KwTo $K$}{
		...
		\nl
		Sample a batch $\{\ve{x}^{(i)}\}_{i=1}^{m}$ from the normal dataset, and a batch $\{\ve{z}^{(i)}\}_{i=1}^{m}$ from the abnormal dataset; collect both to get batch $y=\{y^{(i)}\}_{i=1}^{2m}$ and set their corresponding label as.\\
